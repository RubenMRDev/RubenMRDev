        ...
        <div data-reveal className="border-t border-line">
          {links.map((link) => (
            <a
              ...
              className="group flex items-center gap-5 border-b border-line py-6 transition-colors hover:bg-surface/60"
              aria-label={link.label}
            >
              <span className="text-muted transition-colors group-hover:text-yellow">{link.icon}</span>
              <span className="flex-1">
                <span className="block text-xl font-semibold text-ink">{link.label}</span>
                <span className="font-mono text-xs tracking-wide text-muted">{link.handle}</span>
              </span>
              <span className="text-lg text-muted transition-transform duration-300 group-hover:translate-x-1 group-hover:text-yellow">
                →
              </span>
            </a>
          ))}
